...
// Variável para armazenar a base de conhecimento
let knowledgeBase = [];

// Carrega o arquivo JSON quando o documento é carregado
document.addEventListener('DOMContentLoaded', async () => {
    // Inicializa o chat (já existente no seu código)
    initializeChat();

    // Tenta carregar a base de conhecimento do arquivo JSON
    try {
        const response = await fetch('../chat-boot/chatboot,json'); // Certifique-se de substituir pelo caminho correto
...
const generateResponse = (message) => {
    ...
    if (message.toLowerCase().includes("olá") || message.toLowerCase().includes("oi") || message.toLowerCase().includes("ola")) {
        ...
        setTimeout(() => {
            ...
            const reportLink = '<a href="components/assets/docs/Relatorio.pdf" download>Baixar Relatório</a>';
            chatbox.appendChild(createChatLi(reportLink, "entrada"));
            
            //criando elemento img
            const imgElement = document.createElement('img');
            imgElement.src = "/components/assets/img/dashboard.svg";
...
document.getElementById('loginForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('/components/pages/chat-boot/usuarios.json');
        ...
        if (user) {
            ...
            window.location.href = '/components/pages/administrador/pagadm.html';
        } else {
